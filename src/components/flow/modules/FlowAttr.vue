			...
			<a-tab-pane key="node-attr">
				...
				<template v-if="currentSelect.type == 'node'">
					<a-form layout="horizontal">
						...
						<a-form-item  v-show="specialShow" v-if="setInfo.nodeDesignate=='SPECIAL_USER'||setInfo.nodeDesignate=='RUNTIME_SPECIAL_USER'" label="指定用户" :label-col="formItemLayout.labelCol" :wrapper-col="formItemLayout.wrapperCol">
							<a-col :md="18" :sm="18">
	...
	export default {
		...
		methods: {
			...
			nodeDesignateChange(e){
				...
				that.specialName = '';
				if(nodeDesignate == 'SPECIAL_USER'||nodeDesignate=='RUNTIME_SPECIAL_USER' || nodeDesignate == 'SPECIAL_ROLE')
				{
			...
			select (val) {
				...
				if (this.currentSelect.type == 'link') {
					...
					if(this.currentSelect.type=='node' || this.currentSelect.type == 'join')
					{
						...
						if(this.currentSelect.setInfo.nodeDesignate=='SPECIAL_USER' || this.currentSelect.setInfo.nodeDesignate=='SPECIAL_ROLE'|| this.currentSelect.setInfo.nodeDesignate=='RUNTIME_SPECIAL_USER')
						{
