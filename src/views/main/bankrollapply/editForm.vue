<template>
  <a-modal title="编辑经费认领"
  @okText="Yes"
  :width="1200" :visible="visible" :confirmLoading="confirmLoading" @ok="handleSubmit"
    @cancel="handleCancel">
    ...
    <a-spin :spinning="confirmLoading">
      ...
      <a-form :form="form">
        <a-row :gutter="24">
          <a-col :md="24" :sm="24">
            <a-form-item label="科研系统项目ID" :labelCol="labelCol_JG" :wrapperCol="wrapperCol_JG" has-feedback>
              <a-input placeholder="请输入科研系统项目ID" v-decorator="['applytitle']" />
            </a-form-item>
          </a-col>
        </a-row>
...
import { bankrollapplyEdit,amtofapplying } from '@/api/modular/main/bankrollapplyManage'
...
export default {
  ...
  methods: {
    ...
    edit(record) {
        ...
        this.form.setFieldsValue(
          {
            ...
            syncflg_keyan: record.syncflg_keyan,
            syncflg_caiwu: record.syncflg_caiwu,
      ...
      this.invoiceTitleTitle=record.invoice.invoicetitle,
      this.taxpayerid=record.invoice.taxpayerid,
      this.user_mail=record.invoice.user_mail,
      this.linkmobile=record.invoice.linkphone,
      this.customertype=record.invoice.customertype,
      this.invoicetype=record.invoice.invoicetype,
      this.invoiceitem=record.invoice.invoiceitem,
      ...
      this.form.getFieldDecorator('invoice.invoiceitem', { initialValue: this.invoiceitem}) 
            // this.$forceUpdate()

      this.lendinvoiceFunc(record.lendinvoiceflg)
      this.needinvoiceFunc(record.needinvoice)
    ...
    getAmtOfApplying(record) {
      var queryParam = {'mainkey': record.relateid}
    ...
    lendinvoiceFunc(type) {
      ...
      this.form.getFieldDecorator('lendinvoiceFlg', { initialValue: this.lendinvoiceFlg })
    },
    /**
     * 选择菜单类型执行初始化表单变量
     */
    needinvoiceFunc(type) {
      ...
      this.form.getFieldDecorator('needinvoice', { initialValue: this.needinvoice })
    },
