    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item label="学号或教工号" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
          <a-input placeholder="请输入学号或教工号" v-decorator="['studentid', {rules: [{required: true, message: '请输入学号或教工号！'}]}]" />
        </a-form-item>
        <a-form-item label="姓名" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
          <a-input placeholder="请输入姓名" v-decorator="['studentname', {rules: [{required: true, message: '请输入姓名！'}]}]" />
        </a-form-item>
        <a-form-item label="发票抬头" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
          <a-input placeholder="请输入发票抬头" v-decorator="['invoiceTitle', {rules: [{required: true, message: '请输入发票抬头！'}]}]" />
        </a-form-item>
        <a-form-item label="开具类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select style="width: 100%" placeholder="请选择开具类型" v-decorator="['customerType', {rules: [{ required: true, message: '请选择开具类型！' }]}]">
            <a-select-option v-for="(item,index) in customerTypeData" :key="index" :value="item.code">{{ item.name }}</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="发票类型" :labelCol="labelCol" :wrapperCol="wrapperCol">
          <a-select style="width: 100%" placeholder="请选择发票类型" v-decorator="['invoicetype', {rules: [{ required: true, message: '请选择发票类型！' }]}]">
            <a-select-option v-for="(item,index) in invoicetypeData" :key="index" :value="item.code">{{ item.name }}</a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item label="纳税人识别号" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
          <a-input placeholder="请输入纳税人识别号" v-decorator="['taxpayerid', {rules: [{required: true, message: '请输入纳税人识别号！'}]}]" />
        </a-form-item>
          ...
          this.form.setFieldsValue(
            {
              ...
              customerType: record.customerType.toString(),
              invoicetype: record.invoicetype.toString(),
      ...
      handleSubmit () {
        ...
        validateFields((errors, values) => {
          if (!errors) {
            ...
            invoicetitleEdit(values).then((res) => {
              if (res.success) {
