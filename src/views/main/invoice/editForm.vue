  ...
  export default {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.makeouttypeData = this.$options.filters['dictData']('vis_type')
        this.notifyflgData = this.$options.filters['dictData']('yes_or_no')
        setTimeout(() => {
          this.form.setFieldsValue(
            {
              id: record.id,
              fee_id: record.fee_id,
              feeuser_id: record.feeuser_id,
              order_no: record.order_no,
              trade_no: record.trade_no,
              user_id: record.user_id,
              student_id: record.student_id,
              user_name: record.user_name,
              user_mail: record.user_mail,
              linkphone: record.linkphone,
              invoicetype: record.invoicetype,
              invoicetitle: record.invoicetitle,
              invoiceitem: record.invoiceitem,
              invoicecontent: record.invoicecontent,
              taxpayerid: record.taxpayerid,
              invoiceamount: record.invoiceamount,
              invoicenumber: record.invoicenumber,
              invoicedrawer : record.invoicedrawer ,
              stageflg: record.stageflg,
              buyerbankname: record.buyerbankname,
              buyerbankno: record.buyerbankno,
              buyeraddresss: record.buyeraddresss,
              buyertelephone: record.buyertelephone,
              issuerequestid: record.issuerequestid,
              issueresult: record.issueresult,
              taxcode: record.taxcode,
              rawinvoicecode: record.rawinvoicecode,
              rawinvoicenumber: record.rawinvoicenumber,
              invoicecode: record.invoicecode,
              workflg: record.workflg,
              makeouttype: record.makeouttype,
              makeoutdesc: record.makeoutdesc,
              einvoiceqrcode: record.einvoiceqrcode,
              invoicetax: record.invoicetax,
              invoiceonlytax: record.invoiceonlytax,
              invoicehasnotax: record.invoicehasnotax,
              feesource: record.feesource,
              relatenid: record.relatenid,
              notifymethod: record.notifymethod,
              notifyflg: record.notifyflg,
              notifytimes: record.notifytimes,
              bank: record.bank,
              bankNo: record.bankNo,
              einvoiceflg: record.einvoiceflg,
              einvoiceurl: record.einvoiceurl
            }
          )
        }, 100)
        this.form.getFieldDecorator('invouicetime', { initialValue: moment(record.invouicetime, 'YYYY-MM-DD') })
        this.invouicetimeDateString = moment(record.invouicetime).format('YYYY-MM-DD')
        this.form.getFieldDecorator('inputtime', { initialValue: moment(record.inputtime, 'YYYY-MM-DD') })
        this.inputtimeDateString = moment(record.inputtime).format('YYYY-MM-DD')
      },
      handleSubmit () {
