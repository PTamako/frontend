  ...
  <div>
    ...
    <a-card :bordered="false">
      <s-table
        ...
        :rowClassName="
          record => {
            if (new Date(record.createdTime) < new Date()) {
              return 'red'
            } else {
              return
            }
...
export default {
  ...
  created() {
    ...
    if (
      ...
      this.hasPerm('flcFlowinstance:edit') ||
      this.hasPerm('flcFlowinstance:delete')
    ) {
      this.columns.push({
        title: '操作',
        width: '100px',
  ...
  methods: {
    //月份选择
    handleChange(value) {
      this.queryParam.createdTime = value;
    },
