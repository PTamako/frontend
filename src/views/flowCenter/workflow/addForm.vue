<template>
  ...
  >
      ...
      <x-card>
        <div slot="content" class="table-page-search-wrapper">
          <a-form layout="inline">
            <a-form-item label="下一步审核人员" :labelCol="labelCol" :wrapperCol="wrapperCol" has-feedback>
              <a-col :md="18" :sm="18">
                <a-input :disabled="true" placeholder="请选择下一步审核人员" v-model="nextMakerName" />
              </a-col>
              <a-col
                v-show="
                  nextNodeDesignateType == 'RUNTIME_SPECIAL_USER' || nextNodeDesignateType == 'RUNTIME_SPECIAL_ROLE'
                "
                :md="6"
                :sm="6"
              >
    ...
    <user-special-form ref="userSpecialForm" @ok="handleOk" />
...
import userSpecialForm from '@/views/system/user/userSpecialForm'
...
export default {
  ...
  data() {
    return {
      ...
      nextNodeDesignates: [],
      nextMakerName: '',
      tmpnextNodeDesignates: [],
      tmpnextNodeDesignateName: [],
      webId: null,
  ...
  methods: {
    ...
    edit(record) {
      ...
      getFlcFlowscheme(queryParam).then(res => {
        ...
        this.tmpnextNodeDesignates = res.data.nextNodeDesignates
        this.tmpnextNodeDesignateName = res.data.nextNodeDesignateName
        this.visible = true
    ...
    setUser() {
      if (this.nextNodeDesignateType == 'RUNTIME_SPECIAL_USER') {
        if (!!this.nodeDesignateData) {
          this.$refs.userListForm.userList()
        } else {
          this.$refs.userSpecialForm.userList(this.tmpnextNodeDesignates, this.tmpnextNodeDesignateName)
        }
